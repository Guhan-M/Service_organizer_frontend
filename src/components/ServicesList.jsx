...
/* ─── Sub-components ─────────────────────────────────── */
function SectionHeader({ title, sub, showBtn, onBtnClick }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
      ...
      {showBtn && (
        ...
        >
          See All <MdChevronRight size={16} />
        </button>
      )}
    </div>
  );
...
/* ─── ServiceCard ─────────────────────────────────────── */
function ServiceCard({ name, price, duration, badge, Icon, rating, badgeColor, onClick }) {
...
/* ─── CleaningCard ───────────────────────────────────── */
function CleaningCard({ item, added, onAdd, onClick }) {
  const [hover, setHover] = useState(false);
...
/*
  ...
  • onServiceClick(serviceId) — called when any card is clicked → Dashboard switches to "services" page
*/
function ServicesList({ cartCount, setCartCount, addedItems, setAddedItems, onServiceClick }) {
    ...
    <main style={{ background:"#F8FAFC" }}>
            ...
            <div style={{ display:"flex", gap:12 }}>
              <button
                ...
                style={{
                  ...
                  boxShadow:"0 4px 20px rgba(99,102,241,0.45)", fontFamily:"inherit",
                }}
              >
                Book Now
              </button>
      ...
      <div style={{ maxWidth:1140, margin:"0 auto", padding:"36px 24px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:52 }}>
          {serviceList.map(({ id, name, Icon, color, accent }) => (
            ...
            >
              ...
              <div style={{ fontSize:13, fontWeight:600, color:"#334155", lineHeight:1.3 }}>{name}</div>
            </div>
          ))}
        </div>

        {/* Popular Services — click → AllServicesPage */}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:20, marginBottom:52 }}>
          {popularServices.map(s => (
            <ServiceCard
              key={s.id}
              {...s}
              onClick={() => onServiceClick(s.id)}   // ← navigate to services page
            />
          ))}
        </div>
        ...
        <SectionHeader
          ...
          sub="Professional cleaning for every corner"
          showBtn
          onBtnClick={() => onServiceClick("all")}
        />
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:18, marginBottom:52 }}>
          {cleaningEssentials.map(item => (
            <CleaningCard
              ...
              onAdd={() => handleAdd(item.id)}         // Add button → cart count
              onClick={() => onServiceClick(item.id)}  // card click → services page
            />
          ))}
        </div>
...
//               }} onClick={()=>{handleCheck()}}>Book Now</button>
//               <button style={{
//                 background:"rgba(255,255,255,0.08)", color:"#fff",
//                 border:"1px solid rgba(255,255,255,0.18)", padding:"12px 28px",
//                 borderRadius:10, fontWeight:600, fontSize:14, cursor:"pointer"
//               }} onClick={()=>{handleCheck()}}>View All</button>
//             </div>
//           </div>
//           <div style={{ display:"flex", justifyContent:"center" }}>
//             <img
