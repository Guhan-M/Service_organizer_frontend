...
import {
  ...
  MdFilterList, MdOutlineInfo, MdArrowBack, MdShoppingCart,
} from "react-icons/md";
...
/*
  Props:
  • cartCount, setCartCount, addedItems, setAddedItems — shared cart state
  • onServiceAdd(id)  — called when Add button clicked → Dashboard shows cart
  • onBack()          — called when Back button clicked → Dashboard shows home
  • selectedService   — optionally pre-highlight a service id from dashboard
*/
function AllServicesPage({
  ...
  /* When user clicks "Add" → add to cart AND navigate to cart page */
  const handleAddAndGoCart = (id) => {
    if (!addedItems[id]) {
      setAddedItems(prev => ({ ...prev, [id]: true }));
      setCartCount(prev => prev + 1);
    }
    onServiceAdd?.(id);   // tell Dashboard → switch to cart page
  };
  ...
  return (
    <div style={{ background:"#F8FAFC", minHeight:"100vh", paddingBottom:80 }}>
      ...
      <div style={{
        ...
        padding:"16px 32px", position:"sticky", top:64, zIndex:100,
      }}>
        <div style={{ maxWidth:1140, margin:"0 auto", display:"flex", gap:12, alignItems:"center" }}>
          ...
          <div style={{
            ...
            <input
              ...
              style={{
                ...
                padding:"12px", width:"100%", fontSize:14, color:"#1E293B",
                fontFamily:"inherit",
              }}
            />
          </div>
          ...
          <button style={{
            ...
            border:"1.5px solid #E2E8F0", borderRadius:10, padding:"0 18px", height:46,
            fontWeight:600, color:"#475569", cursor:"pointer", fontFamily:"inherit",
          }}>
            <MdFilterList /> Filter
          </button>
        </div>
      ...
      {/* ── MAIN CONTENT ── */}
      <div style={{ maxWidth:1140, margin:"28px auto", display:"grid", gridTemplateColumns:"220px 1fr", gap:28, padding:"0 24px" }}>

        {/* Sidebar */}
        <aside style={{ position:"sticky", top:144, height:"fit-content" }}>
          <h3 style={{ fontSize:16, fontWeight:800, color:"#0F172A", marginBottom:16 }}>Categories</h3>
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {categories.map(cat => (
              <button
                ...
                style={{
                  textAlign:"left", padding:"11px 14px", borderRadius:10, border:"none",
                  background: activeTab === cat ? "linear-gradient(135deg,#6366F1,#8B5CF6)" : "transparent",
                  color: activeTab === cat ? "#fff" : "#64748B",
                  fontWeight:600, cursor:"pointer", transition:"0.15s", fontFamily:"inherit",
                  fontSize:14,
                }}
                onMouseEnter={e => { if (activeTab !== cat) e.currentTarget.style.background = "#F1F5F9"; }}
        ...
        <section>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:22 }}>
            <h2 style={{ fontSize:22, fontWeight:800, color:"#1E293B", margin:0 }}>
              {activeTab} Services
              <span style={{ fontSize:15, color:"#94A3B8", marginLeft:10, fontWeight:500 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(240px, 1fr))", gap:18 }}>
            {filteredServices.map(service => (
              <ServiceItem
                ...
                onAddAndGoCart={() => handleAddAndGoCart(service.id)}  // Add → go to cart
              />
            ))}
          </div>
        </section>
...
/* ─── Individual Card ─────────────────────────────────── */
function ServiceItem({ service, isAdded, onToggle, onAddAndGoCart }) {
  return (
    <div style={{
      background:"#fff", borderRadius:16, border:"1px solid #E2E8F0",
      padding:20, transition:"0.2s", position:"relative",
      boxShadow: isAdded ? "0 10px 25px rgba(99,102,241,0.1)" : "0 2px 4px rgba(0,0,0,0.04)",
    }}>
      {/* Icon */}
      <div style={{
      ...
      <h4 style={{ fontSize:15, fontWeight:700, color:"#1E293B", margin:"0 0 4px" }}>{service.name}</h4>

      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:2, color:"#F59E0B", fontWeight:700, fontSize:13 }}>
          <MdStar size={15} /> {service.rating}
        </div>
      ...
      <div style={{
        borderTop:"1px dashed #E2E8F0", paddingTop:12,
        display:"flex", justifyContent:"space-between", alignItems:"center",
      }}>
        <div>
          <span style={{ fontSize:11, color:"#94A3B8", display:"block" }}>Starts from</span>
          <span style={{ fontSize:18, fontWeight:800, color:"#0F172A" }}>₹{service.price}</span>
        </div>

        {/* Add button → goes to cart */}
        <button
          onClick={onAddAndGoCart}
          style={{
        ...
        >
          {isAdded
            ? <><MdCheck size={15}/> Added</>
            : <><MdShoppingCart size={15}/> Add</>}
        </button>
      </div>

      <div style={{ position:"absolute", top:18, right:18, color:"#CBD5E1", cursor:"pointer" }}>
        <MdOutlineInfo size={17} />
      </div>
    </div>
  );
}
